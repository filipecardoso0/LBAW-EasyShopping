...
@section('content')
    @include('partials.breadcrumbs', $path = array('Categories' => route('categories'), ' > '.$categorydata->name => route('gamecategories', $categorydata->categoryid)))
    <section class="flex flex-col items-center justify-center mt-14">
        @if($categorygames->count())
            ...
            <section class="self-end m-12">
                {{ $categorygames->links('pagination::tailwind') }}
            </section>
        @else
            <p class="text-center text-amber-400 text-2xl font-semibold mt-12 mb-12">There are no games that belong to this category yet!</p>
        @endif
    </section>
@endsection
